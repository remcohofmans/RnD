...
export const Message = ({ message, isOwnMessage }) => {
  ...
  return (
    <div className={`flex items-center my-2 ${isOwnMessage ? 'justify-end' : 'justify-start'}`}>
      ...
      {isOwnMessage && (
        <p className="text-xs text-rose-500 mr-2">{formattedTimestamp}</p>
      )}
  
      {/* Message Bubble */}
      <div className={`max-w-xs lg:max-w-md px-4 py-2 rounded-xl ${
        isOwnMessage ? 'bg-rose-500 text-white' : 'bg-rose-100 text-rose-800'
      }`}>
        <p>{message.message}</p>
      </div>
      ...
      {!isOwnMessage && (
        <p className="text-xs text-rose-500 ml-2">{formattedTimestamp}</p>
      )}
    </div>
  );
