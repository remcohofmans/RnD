...
const WheelComponent = ({
  ...
  const themeStyles = {
    pink: {
      ...
      closeButton: 'bg-rose-500 hover:bg-rose-600',
    },
    green: {
      ...
      closeButton: 'bg-green-500 hover:bg-green-600',
    },
  };
      ...
      {showWarning && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-xl text-center">
            <h2 className={`text-2xl font-bold mb-4 ${currentTheme.textColor}`}>Spin Limiet Bereikt!</h2>
            <p className={`mb-4 ${currentTheme.textColor}`}>U heeft alle {MAX_SPINS} beschikbare spins gebruikt voor uw {userSubscription} abonnement.</p>
            <button
              onClick={handleCloseWarning}
              className={`text-white px-4 py-2 rounded transition-all duration-300 ${currentTheme.closeButton}`}
            >
              Sluiten
            </button>
          </div>
